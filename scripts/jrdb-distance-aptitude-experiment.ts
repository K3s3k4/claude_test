// 父馬×馬場適性フィルタ(有効性を確認済み)に続き、父馬×距離帯(スプリント/マイル/中距離/長距離)の
// 適性フィルタが単勝バリューベットにさらに効果があるかを検証する。
// 前回の反省を踏まえ、最初から5分割ウォークフォワード検証で実施。
// 参考として「父馬×馬場×距離帯」を組み合わせた場合も検証する(ただしサンプルが細分化されすぎる懸念あり)。
// 実行: npx tsx scripts/jrdb-distance-aptitude-experiment.ts
...
// JRA一般的な距離区分: スプリント(~1400)/マイル(1401-1800)/中距離(1801-2200)/長距離(2201~)
...
type EdgeBet = {
  ...
  win: boolean
  payout: number
  sireDistanceKey: string // `${sireName}|${distanceBucket}`
  sireTrackDistanceKey: string // `${sireName}|${trackCode}|${distanceBucket}`
}
    ...
    for (const [key, horses] of grouped) {
      ...
      for (let i = 0; i < horses.length; i++) {
        ...
        bets.push({
          ...
          sireTrackDistanceKey: sireName && trackCode && bucket ? `${sireName}|${trackCode}|${bucket}` : '',
        })
      }
    }
  }
...
async function main() {
  console.log(`単勝バリューベット(エッジ+${(EDGE_THRESHOLD * 100).toFixed(0)}pt以上)に父馬×距離帯フィルタを重ねた場合の効果を検証\n`)
  ...
  const filterConfigs: { name: string; key: keyof EdgeBet; mult: number }[] = [
    { name: '父馬×距離帯 勝率>=平均×1.1', key: 'sireDistanceKey', mult: 1.1 },
    { name: '父馬×距離帯 勝率>=平均×1.2', key: 'sireDistanceKey', mult: 1.2 },
    { name: '父馬×馬場×距離帯 勝率>=平均×1.1', key: 'sireTrackDistanceKey', mult: 1.1 },
    { name: '父馬×馬場×距離帯 勝率>=平均×1.2', key: 'sireTrackDistanceKey', mult: 1.2 },
  ]
